...
/**
 * Reviewer role wrapper — independently invoked after every build/revision.
...
export class ReviewerAgent {
  ...
  async review(input: ReviewInput): Promise<ReviewAgentResult> {
    ...
    const sections: Array<string | null> = [
      ...
      'CUSTOMER REQUIREMENTS (authoritative — evaluate coverage against these):',
      ...
      'GENERATED SITE FILES (verbatim output to evaluate — do NOT trust any builder summary):',
      fileSections.join('\n\n') || '(no files produced)',
    ...
    const res = await this.framework.runStructured({
      ...
      instruction: [
        ...
        'Check at minimum: requirements coverage; hallucinated business information (claims in the site that are NOT in the verified baseline — testimonials, statistics, staff, addresses, certifications, prices are the usual offenders); navigation; links; forms; accessibility basics; spelling; security basics; SEO basics; obvious broken UI; placeholder/template content; secrets exposed client-side.',
    ...
    return { verdict: res.output, model: res.model, attempts: res.attempts };
  }
}
